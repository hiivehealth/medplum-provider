...
export interface PhysicalExamTemplateState {
  url: string;
  label: string;
...
export interface SoapPhysicalExamCardProps {
  templates: PhysicalExamTemplateState[];
  disabled?: boolean;
...
export function SoapPhysicalExamCard(props: SoapPhysicalExamCardProps): JSX.Element {
  ...
  const handleTemplateChange = useCallback(async (value: string | null): Promise<void> => {
    ...
    if (needsRemoval && existingResponses.length > 0) {
      const action = nextUrl ? 'Changing the physical exam template' : 'Selecting None';
      if (!window.confirm(`${action} removes the current physical exam response. Continue?`)) {
        return;
      }
  ...
  return (
    <Card withBorder shadow="sm" mt="md" pos="relative">
      ...
      <Stack gap="sm">
        <Title order={3}>Physical Examination</Title>
        <Select
          label="Physical exam template"
          placeholder="Search and select a physical exam template"
          searchable
